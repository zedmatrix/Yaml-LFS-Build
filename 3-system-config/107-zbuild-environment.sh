#!/bin/bash
zmsg() { printf "*** %s\n" "$@"; }
cat > /etc/zbuild.d/zbuild-environment.sh << "EOF"
#!bin/bash
# Begin of /etc/zbuild.d/zbuild-environment.sh
...
# End of /etc/zbuild.d/zbuild-environment.sh
EOF
[ -f "/etc/zbuild.d/zbuild-environment.sh" ] && zmsg " Created: zbuild-environment.sh"
