...
inline std::string stars(int x = 30) { return std::string(x, '*'); }
...
void yprintln(std::format_string<Args...> fmt, Args&&... args) {
    ...
    std::println("{}", Label{text, Color::Code::YELLOW, Color::Code::CYAN});
}

template<typename... Args>
void yprintln(Color::Code fg,
    std::format_string<Args...> fmt, Args&&... args) {
    std::string text = std::format(fmt, std::forward<Args>(args)...);
    std::println("{}", Label{text, fg, Color::Code::CYAN});
}
...
public:
	...
	void fail(std::string text = "") {
		...
		std::println("{}{}", Label{text, Color::Code::RED, Color::Code::RED, 2}
					       , Label{m_fail, Color::Code::RED, Color::Code::RED, x}
					 );
	}
	void pass(std::string text = "") {
		uint16_t x = (SCREEN_WIDTH - 6);
		std::println("{}{}", Label{text, Color::Code::GREEN, Color::Code::GREEN, 2}
					       , Label{m_pass, Color::Code::GREEN, Color::Code::GREEN, x}
					 );
	}
private:
